...
async function handleCSSOutput(file, css) {
  ...
  const outfile = isImageTestVarLangPackage(file) ?
    `${outputDir}/ImageTestVarLang.css` :
    `${outputDir}/style.css`;
...
function isImageTestVarLangPackage(file) {
  return path.normalize(file).includes('packages/ImageTestVarLang/');
}
...
async function compileCSS() {
  ...
  const files = await glob('packages/{,@ImageTestVarLang/}*/src/style.scss');
