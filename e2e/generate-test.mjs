...
const packageNames = await fs.readdir(new URL('../packages/@ImageTestVarLang', import.meta.url))
...
const { packages } = await prompts({
  ...
  hint: '@ImageTestVarLang/core is automatically included',
...
const app = dedent`
    ...
    ${packages.map((pgk) => `import ${camelcase(pgk)} from '@ImageTestVarLang/${pgk}'`).join('\n')}
...
const prettyPath = (url) => url.toString().split('ImageTestVarLang', 2)[1]
